...
* Виджет **decorators**:
  ...
  * Содержит декоратор `@log` который принимает в виде аргумента путь к файлу, в который записывает результат декоррируемой 
  функции(есди функция работает- выведет её имя и "ок", если не работает- выведет ошибку и запишет результат)
  * Аргумент декоратора необязательный, если его не указать, результат выведется в консоль, непосредственно.
...
### Тестирование:
  ...
  * Модуль **test_decorators** проверяет поведение декоратора `@log` с помощью простой функции и ее вывода
### Установка:
